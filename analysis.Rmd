...
HTML version available [here](https://arnielarson.github.io/analysis.html)
...
To prepare the data, I removed the columns that I didn't want to train on.  Initially the column summemry statistics and later decided to remove the first 7 columns with user_name, time and windowing informaiton.  
...
```{r basic_data}
dim(pmldata); str(pmldata[,c(1,2,3,4,5,6,7,8,53)])
```
...
It can also be useful to visualize some of the variables with pair plottings.  I did this for several types of variables that might have correlations.  For example the dumbbell accelerations:
...
pp<-preProcess(pmldata)       # default behaivor is to apply centering and scaling
# I also tried using PCA decompositions
pp2<-preProcess(pmldata, method=c("center","scale","pca"),thresh=.90)
pmltraint<-predict(pp,pmltraining)  # this actually applies the transformation
```

My model training routing used the method "cv" which caret uses to do a 5 fold cross validation on the input data to estimate the out of sample error.  Very convenient.
```
ctrl=trainControl(method="cv",number=5)
mx <- train(classe ~ ., data=pmltraint, method="x",trControl=ctrl)
```
Once a model has been trained, it can be tested on the remaining data.  The caret confusion matrix command is useful to get out of sample accuracy and a truth table with Type I and II errors.  

For example, the model mrf4 is a Random Forest model, trained with method="rf", with 40% of the data, and was found to have an accuracy of a little better than 98%.  
